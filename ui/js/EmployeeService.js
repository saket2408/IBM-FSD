...
const buildEmployeeTable = (employees) =>{
    ...
    employees.forEach((emp)=>{
        rows += `<tr>
                    ...
                    <td><a href='#' onclick="updateEmployee('${emp.eEmail}')">Update</a></td>
                </tr>`
    });
...
class EmployeeService {
    ...
    _update(eEmail){
       let empupdate =  JSON.parse(localStorage.getItem(eEmail));
       document.getElementById('eId').value = empupdate.eId;
       document.getElementById('eName').value = empupdate.eName ;
       document.getElementById('eEmail').value = empupdate.eEmail;
       document.getElementById('eDesignation').value = empupdate.eDesignation;
...
const sortEmployee = ()=>{
    let sortcriteria = document.getElementById('sortBy').value;
    ...
    let employees = [];
       ...
       employees=  employees.sort((emp1,emp2)=>{
            if(emp1[sortcriteria] > emp2[sortcriteria]){
                return 1;
            }else if(emp1[sortcriteria] < emp2[sortcriteria]){
                return -1;
            }else{
                return 0;
            }         
